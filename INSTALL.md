...
ScholaPro is a web based application which relies on other facilities such as a web server, PHP server-side scripting, and a PostgreSQL or MySQL/MariaDB database server.

For ScholaPro to work you must first have your web server working, PostgreSQL (or MySQL/MariaDB) working, PHP working (including the `pgsql`, `mysqli`, `pdo`, `gettext`, `intl`, `mbstring`, `gd`, `curl`, `xml` & `zip` extensions). Setting these up varies a lot with operating system so it is well beyond the scope of this brief install document.

ScholaPro was tested on:
...
- [**Windows**](https://gitlab.com/francoisjacquet/scholapro/wikis/How-to-install-ScholaPro-on-Windows)
- [**Mac**](https://gitlab.com/francoisjacquet/scholapro/-/wikis/How-to-install-ScholaPro-on-Mac-(macOS,-OS-X))
- [**cPanel**](https://gitlab.com/francoisjacquet/scholapro/wikis/How-to-install-ScholaPro-on-cPanel)
- [**Softaculous**](https://gitlab.com/francoisjacquet/scholapro/-/wikis/How-to-install-ScholaPro-with-Softaculous)
- [**Docker**](https://github.com/francoisjacquet/docker-scholapro)
- **Ubuntu** (or any Debian-based Linux distribution), see below

If you do not have the technical resources or skills to install ScholaPro, you can subscribe to a hosted plan at https://www.scholapro.com


Installing the package
----------------------

Unzip the [latest version](https://www.scholapro.org/download/) of ScholaPro, or clone the git repository (and checkout the [latest release](https://gitlab.com/francoisjacquet/scholapro/-/releases) tag) to a directory that is accessible to your web browser. Edit the `config.inc.sample.php` file to set the configuration variables and rename it to `config.inc.php`.
...
- `$RosarioPath` Full path to ScholaPro installation.
...
- `define( 'ROSARIO_POST_MAX_SIZE_LIMIT', 16 * 1024 * 1024 );` Limit `$_POST` array size (default is 16MB). More info [here](https://gitlab.com/francoisjacquet/scholapro/-/blob/mobile/Warehouse.php#L322).
...
Now, you're ready to setup the ScholaPro database. If you have access to the command prompt for your server, open a terminal window and follow these instructions.
...
```
2. Create the scholapro user:
```bash
postgres=# CREATE USER scholapro_user WITH PASSWORD 'scholapro_user_password';
```
3. Create the scholapro database:
```bash
postgres=# CREATE DATABASE scholapro_db WITH ENCODING 'UTF8' OWNER scholapro_user;
```
...
```
3. Create the scholapro user:
```bash
mysql> CREATE USER 'scholapro_user'@'localhost' IDENTIFIED BY 'scholapro_user_password';
```
4. Create the scholapro database:
```bash
mysql> CREATE DATABASE scholapro_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci;
mysql> GRANT ALL PRIVILEGES ON scholapro_db.* TO 'scholapro_user'@'localhost';
```
5. Logout of MySQL:
```bash
mysql> \q
...
[Quick Setup Guide](https://www.scholapro.org/quick-setup-guide/)

[Secure ScholaPro](https://gitlab.com/francoisjacquet/scholapro/-/wikis/Secure-ScholaPro)
