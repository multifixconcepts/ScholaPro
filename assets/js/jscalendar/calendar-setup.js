...
Calendar.setup = function (params) {
	...
	var tmp = ["dayField", "monthField", "yearField", "displayArea", "button"];
	for (var i in tmp) {
		if (typeof params[tmp[i]] == "string") {
			params[tmp[i]] = document.getElementById(params[tmp[i]]);
		}
	}
	//if (!(params.flat || params.dayField || params.displayArea || params.button)) {
	...
	function onSelect(cal) {
		...
		if (update && p.monthField) {
			//p.inputField.value = cal.date.print(p.ifFormat);
			for(var i=0;i<p.monthField.length;i++)
			{
				if(p.monthField.options[i].value==cal.date.printMonth().toUpperCase())
				{
					p.monthField.options[i].selected = true;
					break;
				}
			}
			for(var i=0;i<p.dayField.length;i++)
			{
				if(p.dayField.options[i].value==cal.date.printDay())
				{
					p.dayField.options[i].selected = true;
					break;
				}
			}
			for(var i=0;i<p.yearField.length;i++)
			{
				if(p.yearField.options[i].value==cal.date.printYear())
				{
					p.yearField.options[i].selected = true;
					break;
				}
			}

			// Fix regression since 12.5: trigger change event when selecting date from calendar (upstream 12.7.4)
	...
	var triggerEl = params.button || params.displayArea || params.dayField;
	triggerEl["on" + params.eventName] = function() {
		var dateEl = params.dayField || params.displayArea;
