 ...
 *
 * @since 12.0
 * @since 12.2 Send ScholaPro version
 *
...
function AddonInstallationStatisticsPost( $type, $addon_dir )
{
	...
	<script>
		$(document).ready(function(){
			...
			$.ajax({
				type: 'POST',
				url: 'https://www.scholapro.org/addon-statistics/installation-submit.php',
...
function AddonUpsellPremium( $type, $addon_dir, $md_filename = 'PREMIUM.md' )
{
	...
	if ( strpos( $_SERVER['HTTP_HOST'], '.scholapro.com' ) !== false ) // TODO add explanation on login page! "The administrator at your school who signed up to scholapro.com can [login here](). Please contact this person or use the Contact form linked above if you need help."
	{
		if ( User( 'PROFILE' ) !== 'admin' )
		{
			// Only admins should see this Button on scholapro.com.
			return '';
		}

		// If on scholapro.com, replace .org link with .com link
		$lang_2_chars = mb_substr( $_SESSION['locale'], 0, 2 );

		$locale_short = $lang_2_chars === 'fr' || $lang_2_chars === 'es' ?
			$lang_2_chars . '/' : '';

		$com_link = '(https://www.scholapro.com/' . $locale_short . 'switch-to-premium/)';

		$addon_slug = mb_strtolower( str_replace( '_', '-', $addon_dir ) );

		$org_links = [
			'$\(https://www\.scholapro\.org/' . $type . 's/' . $addon_slug . '.*\)$',
			'$\(https://www\.scholapro\.org/' . $lang_2_chars . '/' . $type . 's/' . $addon_slug . '.*\)$',
		];
