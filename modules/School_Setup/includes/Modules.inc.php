...
// Core modules (packaged with ScholaPro):
	...
	&& AllowEdit() )
{
	if ( DeletePrompt( _( 'Module' ) ) )
	{
		...
		// Uploaded (non-core) modules can be deleted whatever their activation state.
		...
		if ( $can_delete )
		{
			...
			// Reload menu: deleted module entries should disappear.
			_reloadMenu();
		}
...
function _makeDelete( $module_title, $activated = null )
{
	...
	// @since 8.0 Add-on disable delete.
	// Uploaded (non-core) modules always get a Delete button, whatever their state.
	$can_delete = ! defined( 'ROSARIO_DISABLE_ADDON_DELETE' ) || ! ROSARIO_DISABLE_ADDON_DELETE;
