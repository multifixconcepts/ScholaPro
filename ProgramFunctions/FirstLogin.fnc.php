...
if ( ! function_exists( 'FirstLoginPoll' ) )
{
	 ...
	 *
	 * @return string Poll HTML array or empty string if not 'admin' user or if scholapro.org not reachable.
	 */
	function FirstLoginPoll()
	{
		...
		$has_connection = @file_get_contents( 'https://www.scholapro.org/installation-poll/poll-submit.php' );
		...
		$fields[] = sprintf(
			...
			URLEscape( 'https://www.scholapro.org/' . $url_lang . 'installation-poll/' )
		);
		...
		$form = '<form action="https://www.scholapro.org/installation-poll/poll-submit.php" method="POST" id="first-login-poll-form" target="_top">';
