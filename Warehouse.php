...
bindtextdomain( 'scholapro', $LocalePath );

// Ensures text returned is utf-8, quite often this is iso-8859-1 by default.
bind_textdomain_codeset( 'scholapro', 'UTF-8' );

// Sets the domain name, this means gettext will be looking for a file called scholapro.mo.
textdomain( 'scholapro' );
 ...
 * Modules
 *
 * Core modules (packaged with ScholaPro): cannot be deleted.
 */
$RosarioCoreModules = [
	'School_Setup',
	'Students',
 ...
 * Plugins
 *
 * Core plugins (packaged with ScholaPro): cannot be deleted.
 */
$RosarioCorePlugins = [
	'Moodle',
];
...
<script>if (!window.$) window.location.href = 'index.php?modfunc=logout&token=' + <?php echo json_encode( $_SESSION['token'] ); ?>;</script>
			...
			// @since 12.3 Cache killer: use file last modified time hash instead of ScholaPro version
...
function isPopup( $modname = '', $modfunc = '' )
{
	...
	trigger_error(
		'isPopup() function is deprecated since ScholaPro 12.0. Use colorBox instead of popup window.',
		E_USER_DEPRECATED
	);
