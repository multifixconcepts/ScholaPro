...
// scholapro.sql files available for database translation.
$locales = [
	'fr_FR.utf8' => 'French',
...
if ( _configTableCheck() )
{
	...
	$addons_sql = $scholapro_sql = '';

	// @since 12.3 Move SQL translation file to locale/[locale_code]/ folder
	$scholapro_sql_file = 'locale/' . $_POST['locale'] . '/scholapro.sql';

	$addons_sql_file = 'locale/' . $_POST['locale'] . '/scholapro_addons.sql';

	// Translate Database.
	if ( file_exists( $scholapro_sql_file ) )
	{
		// Same translation files for both MySQL & PostgreSQL.
		$scholapro_sql = file_get_contents( $scholapro_sql_file );
	...
	die( 'Success: database translated. <a href="index.php">Access ScholaPro</a>' );
}

$sql_file = $DatabaseType === 'mysql' ? 'scholapro_mysql.sql' : 'scholapro.sql';
...
$scholapro_sql = file_get_contents( $sql_file );

if ( $DatabaseType === 'mysql' )
{
	// @since 10.0 Remove DELIMITER $$ declarations before procedures or functions.
	$scholapro_sql = MySQLRemoveDelimiter( $scholapro_sql );
...
if ( file_exists( 'scholapro_addons.sql' ) )
{
	// @since 5.1 Install add-ons.
	// Same add-ons file for both MySQL & PostgreSQL.
	$addons_sql = _getAddonsSQL( 'scholapro_addons.sql' );
...
die( 'Success: database' .
	( file_exists( 'scholapro_addons.sql' ) ? ' and add-ons' : '' ) .
	' installed. <a href="index.php">Access ScholaPro</a>' );
